...
$input = [
  ...
  'category' => $event['ambito'] ?? '',
  'startDateTime' => '',
  'endDateTime' => '',
    'sede_nome' => $event['sede_nome'] ?? '',
    'sede_via' => $event['via'] ?? '',
    'sede_citta' => $event['citta'] ?? '',
    'sede_provincia' => $event['provincia'] ?? '',
    'contatto_email' => $event['email'] ?? '',
    'contatto_telefono' => $event['telefono'] ?? '',
    'media_nome' => array_column($eventBundle['multimedia'], 'nome'),
    'media_tipo' => array_column($eventBundle['multimedia'], 'tipoFile'),
    ...
    <section class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <input type="text" name="category" value="<?php echo old_edit($input, 'category'); ?>" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" required>
        </div>
    ...
    <section class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input type="text" name="sede_nome" value="<?php echo old_edit($input, 'sede_nome'); ?>" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" required>
        </div>
        <div>
        ...
        <div>
          ...
          <input type="text" name="sede_citta" value="<?php echo old_edit($input, 'sede_citta'); ?>" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" required>
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700">Provincia</label>
          <input type="text" name="sede_provincia" value="<?php echo old_edit($input, 'sede_provincia'); ?>" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" required>
        </div>
      </div>
    </section>
    ...
    <section class="space-y-3">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input type="email" name="contatto_email" value="<?php echo old_edit($input, 'contatto_email'); ?>" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" required>
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700">Telefono</label>
          <input type="text" name="contatto_telefono" value="<?php echo old_edit($input, 'contatto_telefono'); ?>" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2">
        </div>
      </div>
    </section>
